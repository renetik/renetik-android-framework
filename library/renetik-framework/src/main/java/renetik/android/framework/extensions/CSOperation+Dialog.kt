...
fun <Data : Any> CSOperation<Data>.sendWithProgress(title: String, onSuccess: ((Data) -> Unit)? = null)
        : CSOperation<Data> = apply {
    val process = send()

    val progress = navigation.dialog(title)
        .showProgress(getString(R.string.renetik_android_framework_send_request_cancel)) { cancel() }
