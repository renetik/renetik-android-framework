...
class CSValueStore(name: String) : CSContextController() {
    ...
    fun <T : CSJsonData> loadDataList(type: KClass<T>, key: String) =
            createList(type, loadJson(key) as List<CSMap<String, Any?>>?)

    fun <T : CSJsonData> loadDataList(type: KClass<T>, key: String, default: List<T>) =
            createList(type, loadJson(key) as List<CSMap<String, Any?>>?, default)
