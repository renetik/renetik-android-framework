...
abstract class CSViewController<ViewType : View> : CSView<ViewType>, CSViewControllerParent {
    ...
    var isStarted = CSLang.NO
    var isCreated = CSLang.NO
    var isResumed = CSLang.NO
    var isResumeFirstTime = CSLang.NO
    var isPaused = CSLang.NO
    var isDestroyed = CSLang.NO
    ...
    private var isShowing = CSLang.NO
    private var onViewShowingCalled = CSLang.NO
    ...
    protected fun onBack(goBack: CSValue<Boolean>) {
        onBack.fire(goBack)
        if (goBack.value && isShowing) goBack.value = onGoBack()
    }

    protected open fun onGoBack() = CSLang.YES
    ...
    protected fun updateVisibilityChanged() {
        if (checkIfIsShowing()) {
            if (!isShowing) onViewVisibilityChanged(CSLang.YES)
        } else if (isShowing) onViewVisibilityChanged(CSLang.NO)
    }

    protected fun checkIfIsShowing(): Boolean {
        if (!isResumed) return CSLang.NO
        if (showingInContainer == CSLang.NO) return CSLang.NO
        if (parentController?.isShowing == CSLang.NO) return CSLang.NO;
        return CSLang.YES
    }
    ...
    private fun onViewVisibilityChanged(showing: Boolean) {
        isShowing = showing
        if (isShowing) {
            isVisibleEventRegistrations.setActive(CSLang.YES)
            invalidateOptionsMenu()
            onViewShowing()
        } else {
            isVisibleEventRegistrations.setActive(CSLang.NO)
            invalidateOptionsMenu()
            onViewHiding()
            whileShowingEventRegistrations.cancel()
        }
    ...
    protected open fun onViewShowing() {
        if (!onViewShowingCalled) {
            ...
            onViewShowingCalled = CSLang.YES
