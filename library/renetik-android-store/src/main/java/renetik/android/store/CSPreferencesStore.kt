...
class CSPreferencesStore(id: String) : CSContext(), renetik.android.store.CSStore {

    private val preferences = getSharedPreferences(id, Context.MODE_PRIVATE)
    override val eventChanged = event<renetik.android.store.CSStore>()
    ...
    override fun load(store: renetik.android.store.CSStore) = with(preferences.edit()) {
        loadAll(store)
        eventChanged.fire(this@CSPreferencesStore)
        apply()
    }

    override fun reload(store: renetik.android.store.CSStore) = with(preferences.edit()) {
        clear()
        loadAll(store)
        eventChanged.fire(this@CSPreferencesStore)
        apply()
