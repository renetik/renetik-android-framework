...
import cs.android.view.adapter.OnPageChange;
...
public class PagerController extends CSViewController {

    private CSList<String> _titles;
    private CSViewController[] _views;
    private CSView _emptyView;

    public PagerController(CSViewController parent, int pagerId, CSList<String> titles, CSViewController... views) {
    ...
    protected void onCreate() {
        super.onCreate();
        asPager().setAdapter(new CSPagerAdapter(_titles, _views));
        asPager().addOnPageChangeListener(new OnPageChange() {
